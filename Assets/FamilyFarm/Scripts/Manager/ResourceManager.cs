...
namespace LuaFramework {
    public class ResourceManager : Manager {
        ...
        public delegate void ImageLoadCallBack(Texture2D texture);
        ...
        public void Initialize() {
            ...
            byte[] obj = File.ReadAllBytes(Util.DataPath + "en_US.config");
            obj = ZlibStream.UncompressBuffer(obj);
            string str = System.Text.Encoding.Default.GetString(obj);
            config = JsonMapper.ToObject(str);
            //Debug.Log(config["store"]["5"]["name"]);
            //Debug.Log(str);

            if (!Directory.Exists(Util.ImageCachePath))
            {
                Directory.CreateDirectory(Util.ImageCachePath);
            }
        }

        public JsonData GetItemConf(string itemId)
        {
            return config["store"][itemId];
        }

        public JsonData GetShopData()
        {
            return config["store"];
        }
        ...
        {
            if (!File.Exists(Util.ImageCachePath + url))
            {
                //如果之前不存在缓存文件  
        ...
        IEnumerator DownloadImage(string url, ImageLoadCallBack callBack)
        {
            Debug.Log("downloading new image:" + AppConst.HOST + "images/" + url);//url转换HD5作为名字  
            WWW www = new WWW("https://farm-dev525.socialgamenet.com/en/static/images/" + url);
            yield return www;
            ...
            File.WriteAllBytes(Util.ImageCachePath + url, pngData);
        ...
        IEnumerator LoadLocalImage(string url, ImageLoadCallBack callBack)
        {
            string filePath = "file:///" + Util.ImageCachePath + url;
